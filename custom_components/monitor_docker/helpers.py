...
from .const import ATTR_ONLINE_CPUS
from .const import ATTR_VERSION_ARCH
from .const import ATTR_VERSION_KERNEL
from .const import ATTR_VERSION_OS
from .const import ATTR_VERSION_OS_TYPE
...
from .const import CONTAINER_INFO_HEALTH
from .const import CONTAINER_INFO_IMAGE
from .const import CONTAINER_INFO_NETWORK_AVAILABLE
from .const import CONTAINER_INFO_STATE
from .const import CONTAINER_INFO_STATUS
from .const import CONTAINER_INFO_UPTIME
from .const import CONTAINER_STATS_1CPU_PERCENTAGE
from .const import CONTAINER_STATS_CPU_PERCENTAGE
from .const import CONTAINER_STATS_MEMORY
from .const import CONTAINER_STATS_MEMORY_PERCENTAGE
from .const import CONTAINER_STATS_NETWORK_SPEED_DOWN
from .const import CONTAINER_STATS_NETWORK_SPEED_UP
from .const import CONTAINER_STATS_NETWORK_TOTAL_DOWN
from .const import CONTAINER_STATS_NETWORK_TOTAL_UP
from .const import DOCKER_INFO_CONTAINER_PAUSED
from .const import DOCKER_INFO_CONTAINER_RUNNING
from .const import DOCKER_INFO_CONTAINER_STOPPED
from .const import DOCKER_INFO_CONTAINER_TOTAL
from .const import DOCKER_INFO_IMAGES
from .const import DOCKER_INFO_VERSION
from .const import DOCKER_STATS_1CPU_PERCENTAGE
from .const import DOCKER_STATS_CPU_PERCENTAGE
from .const import DOCKER_STATS_MEMORY
from .const import DOCKER_STATS_MEMORY_PERCENTAGE
...
#################################################################
class DockerAPI:
    """Docker API abstraction allowing multiple Docker instances beeing monitored."""   # noqa: E501
    ...
    def __init__(self, hass, config, startCount=0):
        ...
        try:
            ...
            # When we reconnect with tcp, we should delay - docker is maybe not fully ready  # noqa: E501
            ...
            if url is not None:
                ...
                if url.find('tcp:') == 0 or url.find('http:') == 0:
                    ...
                    if tlsverify is None:
                        _LOGGER.debug(
                            "[%s]: Docker environment 'DOCKER_TLS_VERIFY' is NOT set",  # noqa: E501
                            self._instance,
                        )
                    else:
                        _LOGGER.debug(
                            "[%s]: Docker environment set 'DOCKER_TLS_VERIFY=%s'",  # noqa: E501
                            self._instance,
                            tlsverify,
                        )
                    ...
                    if certpath is None:
                        _LOGGER.debug(
                            "[%s]: Docker environment 'DOCKER_CERT_PATH' is NOT set",  # noqa: E501
                            self._instance,
                        )
                    else:
                        _LOGGER.debug(
                            "[%s]: Docker environment set 'DOCKER_CERT_PATH=%s'",  # noqa: E501
                            self._instance,
                            certpath,
                        )
                    ...
                    if self._config[CONF_CERTPATH]:
                        _LOGGER.debug(
                            "[%s]: Docker CertPath set '%s', setting environment variables DOCKER_TLS_VERIFY/DOCKER_CERT_PATH",  # noqa: E501
                            self._instance,
                            self._config[CONF_CERTPATH],
                        )
                        os.environ['DOCKER_TLS_VERIFY'] = '1'
                        os.environ['DOCKER_CERT_PATH'] = self._config[CONF_CERTPATH]  # noqa: E501
        ...
        for container in containers or []:
            ...
            _LOGGER.debug(
                '[%s] %s: Container Monitored',
                self._instance, cname,
            )
    ...
    async def _run_docker_events(self):
        """Function to retrieve docker events. We can add or remove monitored containers."""  # noqa: E501
        ...
        try:
            ...
            while True:
                ...
                if event is None:
                    _LOGGER.error(
                        '[%s]: run_docker_events loop ended', self._instance,
                    )
                    ...
                    # Remove all the sensors/switches, they will be auto created if connection is working again  # noqa: E501
                ...
                if event['Type'] == CONTAINER:
                    if event['Action'] == 'create':
                        # Check if another task is running, ifso, we don't create a new one  # noqa: E501
                        taskcreated = (
                            True if self._event_create
                            or self._event_destroy else False
                        )

                        cname = event['Actor']['Attributes']['name']

                        # Add container name to containers to be monitored this has to  # noqa: E501
                        # be a new task, otherwise it will block our event monitoring  # noqa: E501
                        if cname not in self._event_create:
                            _LOGGER.debug(
                                '[%s] %s: Event create container',
                                self._instance,
                                cname,
                            )
                            ...
                            _LOGGER.error(
                                '[%s] %s: Event create container, but already in working table?',  # noqa: E501
                                self._instance,
                                cname,
                            )
                        ...
                        if self._event_create and not taskcreated:
                            self._loop.create_task(
                                self._container_create_destroy(),
                            )
                        ...
                        # Remove container name to containers to be monitored this has to  # noqa: E501
                        # be a new task, otherwise it will block our event monitoring  # noqa: E501
                        if cname in self._event_create:
                            _LOGGER.warning(
                                "[%s] %s: Event destroy received, but create wasn't executed yet",  # noqa: E501
                                self._instance,
                                cname,
                            )
                            ...
                            _LOGGER.error(
                                '%s: Event destroy container, but already in working table?',  # noqa: E501
                                cname,
                            )
                        ...
                        # during a docker-compose up -d <container> the old container can be renamed  # noqa: E501
                        # sensors/switch should be removed before the new container is monitored  # noqa: E501
                        ...
                        if oname in self._containers:
                            ...
                            # We also should remove entities, rename does not work  # noqa: E501
                            self._containers[cname].remove_entities()

                        else:
                            _LOGGER.error(
                                "[%s] %s: Event rename container doesn't exist in list?",  # noqa: E501
                                self._instance,
                                oname,
                            )
    ...
    async def _container_add(self, cname):
        ...
        if cname in self._containers:
            _LOGGER.error(
                '[%s] %s: Container already monitored',
                self._instance, cname,
            )
            return

        _LOGGER.debug(
            '[%s] %s: Starting Container Monitor',
            self._instance, cname,
        )
        ...
        if result:
            ...
            _LOGGER.error(
                '[%s] %s: Problem during start of monitoring',
                self._instance,
                cname,
            )
    ...
    async def _container_remove(self, cname):
        ...
        if cname in self._containers:
            _LOGGER.debug(
                '[%s] %s: Stopping Container Monitor',
                self._instance, cname,
            )
            ...
            _LOGGER.error(
                '[%s] %s: Container is NOT monitored',
                self._instance, cname,
            )
        ...
        try:
            while True:
                ...
                if self._dockerStopped:
                    _LOGGER.debug(
                        '[%s]: Stopping docker info thread', self._instance,
                    )
                    break
                ...
                self._info[DOCKER_INFO_CONTAINER_PAUSED] = info.get(
                    'ContainersPaused',
                )
                self._info[DOCKER_INFO_CONTAINER_STOPPED] = info.get(
                    'ContainersStopped',
                )
                self._info[DOCKER_INFO_CONTAINER_TOTAL] = info.get(
                    'Containers',
                )
                ...
                for container in self._containers.values():
                    try:
                        ...
                        if info.get(CONTAINER_INFO_STATE) == 'running':
                            ...
                            if stats.get(CONTAINER_STATS_CPU_PERCENTAGE) is not None:  # noqa: E501
                                self._info[DOCKER_STATS_CPU_PERCENTAGE] += stats.get(  # noqa: E501
                ...
                _LOGGER.debug(
                    '[%s]: Version: %s, Containers: %s, Running: %s, CPU: %s%%, 1CPU: %s%%, Memory: %sMB, %s%%',  # noqa: E501
                    self._instance,
    ...
    def get_container(self, cname):
        if cname in self._containers:
            ...
            _LOGGER.error(
                '[%s]: Trying to get a not existing container %s',
                self._instance,
                cname,
            )
            return None
...
class DockerContainerAPI:
        ...
        while True:
            try:
                ...
                if not self._busy:
                    ...
                    if self._info[CONTAINER_INFO_STATE] in ('running', 'paused'):  # noqa: E501
                ...
                _LOGGER.debug(
                    '[%s] %s: Container received concurrent.futures._base.CancelledError',  # noqa: E501
                    self._instance,
                    self._name,
                )
                pass
    ...
    async def _run_container_info(self):
        ...
        if CONTAINER_INFO_NETWORK_AVAILABLE not in self._info:
            self._info[CONTAINER_INFO_NETWORK_AVAILABLE] = (
                False if raw['HostConfig']['NetworkMode'] in [
                    'host', 'none',
                ] else True
            )

        try:
            self._info[CONTAINER_INFO_HEALTH] = raw['State']['Health']['Status']  # noqa: E501
        ...
        if self._info[CONTAINER_INFO_STATE] == 'running':
            ...
            self._info[CONTAINER_INFO_STATUS] = 'Exited ({}) {} ago'.format(
                raw['State']['ExitCode'],
                self._calcdockerformat(
                    parser.parse(raw['State']['FinishedAt']),
                ),
            )
        ...
        if self._info[CONTAINER_INFO_STATE] in ('running', 'paused'):
            self._info[CONTAINER_INFO_UPTIME] = dt_util.as_local(
                startedAt,
            ).isoformat()
        else:
    ...
    async def _run_container_stats(self):
        ...
        try:
            ...
            if self._cpu_old:
                ...
                system_delta = float(
                    cpu_new['system'] - self._cpu_old['system'],
                )
        ...
        try:
            ...
            memory_stats['usage_percent'] = round(
                ...
                float(memory_stats['limit']) * 100.0,
                PRECISION,
            )
            ...
            if self._memory_error == 0:
                _LOGGER.error(
                    '[%s] %s: Cannot determine memory usage for container (%s)',  # noqa: E501
                    self._instance,
                    self._name,
                    str(err),
                )
        ...
        if (
            ...
            mem_diff = abs(
                (memory_stats['usage'] / self._memory_prev) - 1,
            ) * 100
        ...
        if self._info[CONTAINER_INFO_NETWORK_AVAILABLE]:
            try:
                ...
                if self._network_old:
                    tx = network_new['total_tx'] - \
                        self._network_old['total_tx']
                    rx = network_new['total_rx'] - \
                        self._network_old['total_rx']
                    tim = (
                    ...
                    network_stats['speed_tx'] = toKB(
                        float(tx) /
                        tim, self._config[CONF_PRECISION_NETWORK_KB],
                    )
                ...
                network_stats['total_tx'] = toMB(
                    ...
                    self._config[CONF_PRECISION_NETWORK_MB],
                )
                ...
                _LOGGER.error(
                    '[%s] %s: Can not determine network usage for container (%s)',  # noqa: E501
                    self._instance,
                    self._name,
                    str(err),
                )
                ...
                # Check how many times we got a network error, after 5 times it won't happen  # noqa: E501
                # anymore, thus we disable error reporting
                self._network_error += 1
                if self._network_error > 5:
                    _LOGGER.error(
                        "[%s] %s: Too many errors on 'networks' stats, disabling monitoring",  # noqa: E501
                        self._instance,
                        self._name,
                    )
        ...
        stats[CONTAINER_STATS_MEMORY_PERCENTAGE] = memory_stats.get(
            'usage_percent',
        )
        stats[CONTAINER_STATS_NETWORK_SPEED_UP] = network_stats.get('speed_tx')
        stats[CONTAINER_STATS_NETWORK_SPEED_DOWN] = network_stats.get(
            'speed_rx',
        )
        stats[CONTAINER_STATS_NETWORK_TOTAL_UP] = network_stats.get('total_tx')
        stats[CONTAINER_STATS_NETWORK_TOTAL_DOWN] = network_stats.get(
            'total_rx',
        )
    ...
    def cancel_task(self):
        if self._task is not None:
            ...
            _LOGGER.info(
                '[%s] %s: Task (not running) can not be cancelled for container info/stats',  # noqa: E501
                self._instance,
                self._name,
            )
    ...
    def rename_entities_containername(self):
        if len(self._subscribers) > 0:
            _LOGGER.debug(
                '[%s] %s: Renaming entities for container',
                self._instance,
                self._name,
            )
    ...
    def remove_entities(self):
        if len(self._subscribers) > 0:
            _LOGGER.debug(
                '[%s] %s: Removing entities from container',
                self._instance,
                self._name,
            )
    ...
    def _calcdockerformat(dt):
        """Calculate datetime to Docker format, because it isn't available in stats."""  # noqa: E501
        if dt is None:
            return 'None'
        ...
        if delta.years != 0:
            return '{} {}'.format(
                delta.years,
                'year' if delta.years == 1 else 'years',
            )
        elif delta.months != 0:
            return '{} {}'.format(
                delta.months,
                'month' if delta.months == 1 else 'months',
            )
        elif delta.days != 0:
            return '{} {}'.format(
                delta.days,
                'day' if delta.days == 1 else 'days',
            )
        elif delta.hours != 0:
            return '{} {}'.format(
                delta.hours,
                'hour' if delta.hours == 1 else 'hours',
            )
        elif delta.minutes != 0:
            return '{} {}'.format(
                delta.minutes,
                'minute' if delta.minutes == 1 else 'minutes',
            )

        return '{} {}'.format(
            delta.seconds,
            'second' if delta.seconds == 1 else 'seconds',
        )
