...
| url                  | string       (Optional)  | Host URL of Docker daemon. Defaults to `unix:///var/run/docker.sock`. When using a unix socket, please add 3 slashes after the `unix:`. Remote Docker daemon is also supported (but untested), use e.g. `tcp://ip:2376/` |
